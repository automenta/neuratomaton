#!/usr/bin/env python3
"""
Phase A: Final Scaling Validation with Optimal Training
"""
import os
import sys
import json
...
SCALING_CONFIGS = [
    {'name': 'small', 'd_model': 64, 'num_layers': 2, 'state_dim': 64, 'epochs': 20, 'lr': 1e-3},
    {'name': 'medium', 'd_model': 128, 'num_layers': 3, 'state_dim': 128, 'epochs': 30, 'lr': 3e-4},
    {'name': 'large', 'd_model': 256, 'num_layers': 4, 'state_dim': 256, 'epochs': 40, 'lr': 1e-4},
]
...
print("="*70)
print("PHASE A: FINAL SCALING VALIDATION")
print("="*70)
...
for scale_cfg in SCALING_CONFIGS:
    name = scale_cfg['name']
    print(f"\n{'='*70}")
    print(f"Scale: {name} (d_model={scale_cfg['d_model']}, layers={scale_cfg['num_layers']}, lr={scale_cfg['lr']})")
    print(f"{'='*70}")
    ...
    for ablation_name, flags in CONFIGS.items():
        ...
        for seed in [42, 123]:
            ...
            acc = train_eval(model, num_kv=8, epochs=scale_cfg['epochs'], lr=scale_cfg['lr'])
            accs.append(acc)
...
with open('archive/experiments/phaseA_scaling_v2.json', 'w') as f:
...
print(f"\nResults saved to: archive/experiments/phaseA_scaling_v2.json")
