...
Verify synergy persists at larger scales (100K -> 400K -> 1.5M params)
"""
import os
import sys
import json
...
def train_eval(model, num_kv, epochs=20, lr=1e-3):
...
SCALING_CONFIGS = [
    {'name': 'small', 'd_model': 64, 'num_layers': 2, 'state_dim': 64, 'expected_params': '~100K'},
    {'name': 'medium', 'd_model': 128, 'num_layers': 3, 'state_dim': 128, 'expected_params': '~400K'},
    {'name': 'large', 'd_model': 256, 'num_layers': 4, 'state_dim': 256, 'expected_params': '~1.5M'},
]
...
print("="*70)
print("PHASE A: SCALING VALIDATION")
print("="*70)
...
for scale_cfg in SCALING_CONFIGS:
    ...
    for ablation_name, flags in CONFIGS.items():
        ...
        for seed in [42, 123, 456]:
            ...
            acc = train_eval(model, num_kv=8, epochs=20)
            accs.append(acc)
    ...
    single_best = max(
        results[name]['ablations'][c]['mean'] 
        for c in ['controller', 'hololink']
    )
...
with open('archive/experiments/phaseA_scaling.json', 'w') as f:
...
print(f"\nResults saved to: archive/experiments/phaseA_scaling.json")
