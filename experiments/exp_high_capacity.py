#!/usr/bin/env python3
"""
Test Synergy at Higher Capacity (12-16 KV pairs)
"""
import os
import sys
import json
...
class QuickMultiKV(data.Dataset):
    def __init__(self, size=600, num_kv=12, min_noise=3, max_noise=10):
        self.data = []
...
def train_eval(model, num_kv, epochs, lr):
    ds = QuickMultiKV(size=800, num_kv=num_kv)
...
print("="*70)
print("SYNERGY AT HIGH CAPACITY (12 KV pairs)")
print("="*70)

results = {}

for scale_name, scale_cfg in [
    ('medium', {'d_model': 128, 'num_layers': 3, 'state_dim': 128, 'epochs': 40, 'lr': 2e-4}),
    ('large', {'d_model': 256, 'num_layers': 4, 'state_dim': 256, 'epochs': 50, 'lr': 1e-4}),
]:
    print(f"\n{'='*70}")
    print(f"Scale: {scale_name}")
    print(f"{'='*70}")
    
    results[scale_name] = {'ablations': {}}
    ...
    for ablation_name, flags in CONFIGS.items():
        ...
        for seed in [42, 123]:
            ...
            acc = train_eval(model, num_kv=12, epochs=scale_cfg['epochs'], lr=scale_cfg['lr'])
            accs.append(acc)
        
        mean_acc = sum(accs) / len(accs)
        results[scale_name]['ablations'][ablation_name] = {'mean': mean_acc, 'params': params}
        print(f"    {ablation_name}: {mean_acc*100:.1f}% ({params:,} params)")
    
    single_best = max(results[scale_name]['ablations'][c]['mean'] for c in ['controller', 'hololink'])
    full_mean = results[scale_name]['ablations']['full']['mean']
    synergy = full_mean - single_best
    results[scale_name]['synergy'] = synergy
    print(f"\n  Synergy: +{synergy*100:.1f}%")

print("\n" + "="*70)
print("SUMMARY: 12 KV PAIRS")
print("="*70)
